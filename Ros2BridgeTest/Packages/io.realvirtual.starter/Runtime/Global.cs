...
namespace realvirtual
{
    ...
    public static class Global
    {
        ...
        private static bool _missingLayerWarned = false;

        //! Sets the layer of a GameObject to the named layer, but only if that layer exists in the project.
        //! This prevents the "A game object can only be in one layer. The layer needs to be in the range [0...31]"
        //! error that occurs when the realvirtual layers (rvTransport, rvMU, ...) have not been configured yet -
        //! for example in a fresh project where the standard settings were never applied.
        //! Returns true if the layer existed and was applied, false if it was missing (and skipped).
        public static bool SetLayerSafe(GameObject go, string layerName)
        {
            if (go == null)
                return false;
            ...
            if (layer < 0)
            {
                WarnMissingLayersOnce(layerName);
                return false;
            }
        ...
        //! Emits a single actionable warning when a realvirtual layer is missing, so a fresh project
        //! does not spam the console. Tells the user exactly how to fix it.
        private static void WarnMissingLayersOnce(string layerName)
        {
            if (_missingLayerWarned)
                return;
            _missingLayerWarned = true;
            Logger.Warning(
                $"realvirtual layers are not configured (missing layer '{layerName}'). " +
                "Open Tools → realvirtual → Get Started and apply the standard settings to set them up.",
                null);
        }
        ...
        #region Professional Extension Hooks

        /// <summary>
        /// Delegate for Professional package to check if a behavior type should be excluded from view type checks.
        /// Used by IsGame4AutomationViewTypeIncluded to exclude organizational components like CAD.
        /// </summary>
        public static Func<realvirtualBehavior, bool> IsExcludedViewType;
        ...
        public static bool IsGame4AutomationViewTypeIncluded(GameObject target)
        {
            try
            {
                ...
                {
                    // Group is always excluded (organizational component)
                    if (behav.GetType() == typeof(Group))
                    {
                        found++;
                    }
                    // Allow Professional package to exclude additional types (e.g., CAD)
                    else if (IsExcludedViewType?.Invoke(behav) == true)
                    {
                        found++;
                    }
                }
        ...
        private static bool _versionSet = false;
...
#if UNITY_EDITOR
            // In Editor: read from package.json (single source of truth)
            try
            {
                string packageJsonPath = System.IO.Path.Combine(
                    System.IO.Path.GetDirectoryName(Application.dataPath),
                    "Packages", "io.realvirtual.professional", "package.json");
                if (!System.IO.File.Exists(packageJsonPath))
                {
                    packageJsonPath = System.IO.Path.Combine(
                        System.IO.Path.GetDirectoryName(Application.dataPath),
                        "Packages", "io.realvirtual.starter", "package.json");
                }
                if (System.IO.File.Exists(packageJsonPath))
                {
                    ...
                    if (match.Success)
                    {
                        string pkgVersion = match.Groups[1].Value; // e.g. "6.3.1-beta.8650" or "6.3.0"

                        // Also read ScriptableObject for changeset number (always available)
                        realvirtualVersion sv = UnityEngine.Resources.Load<realvirtualVersion>("realvirtualVersion");
                        string changeset = sv != null ? sv.Build.ToString() : "";

                        // If package.json already contains changeset (pre-release), extract Build from it
                        if (pkgVersion.Contains("-"))
                        {
                            int lastDot = pkgVersion.LastIndexOf('.');
                            if (lastDot >= 0 && int.TryParse(pkgVersion.Substring(lastDot + 1), out int buildNum))
                                Build = buildNum.ToString();
                            else
                                Build = changeset;
                        }
                        else
                        ...
                        // Show version with changeset: "6.3.0 CS:8650 (Unity 6000.3.5f1)"
                        if (!string.IsNullOrEmpty(changeset) && changeset != "0" && !pkgVersion.Contains(changeset))
                            Version = pkgVersion + " CS:" + changeset + " (Unity " + Application.unityVersion + ")";
                        else
                            Version = pkgVersion + " (Unity " + Application.unityVersion + ")";

                        return;
                    }
                }
            }
            catch { /* fall through to ScriptableObject fallback */ }
#endif
            // Runtime builds (or Editor fallback): read from ScriptableObject baked into Resources
            realvirtualVersion scriptableversion =
                UnityEngine.Resources.Load<realvirtualVersion>("realvirtualVersion");
            if (scriptableversion != null)
            {
                Build = scriptableversion.Build.ToString().Replace("\n", "");
