...
namespace realvirtual
{
    ...
    public class realvirtualBehavior : MonoBehaviour
    {
        ...
        [HideInInspector] public bool IsNetworkOwner = true; //!< True when this component owns its simulation (local authority). Set to false by MultiplayerWEB on remote clients.
        // Logger is now handled by the static Logger class

        //! Called when network ownership changes. Override to self-manage multiuser behavior.
        //! Use SetNetworkOwner() to change ownership — it sets IsNetworkOwner and calls this.
        public virtual void OnOwnershipChanged(bool isOwner) { }

        //! Sets IsNetworkOwner and triggers OnOwnershipChanged callback.
        public void SetNetworkOwner(bool isOwner)
        {
            IsNetworkOwner = isOwner;
