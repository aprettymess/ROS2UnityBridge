...
namespace realvirtual
{
    ...
    public static class Groups
    {
        ...
        private static int _groupCacheVersion = 0;

        //! Initializes the Group cache and registers hierarchy/play mode change listeners.
        [UnityEditor.InitializeOnLoadMethod]
        private static void InitGroupCache()
        {
            UnityEditor.EditorApplication.hierarchyChanged += () => { _groupCacheValid = false; _groupCacheVersion++; };
            UnityEditor.EditorApplication.playModeStateChanged += _ => { _groupCacheValid = false; _groupCacheVersion++; };
        }

        //! Returns a version number that increments whenever the group cache is invalidated.
        //! Consumers can compare this to avoid redundant per-frame queries.
        public static int GetCachedGroupsVersion() => _groupCacheVersion;

        //! Forces the next group query to re-scan the scene and bumps the cache version. Call this right
        //! after adding or removing Group components in the same frame: component add/remove does NOT raise
        //! EditorApplication.hierarchyChanged, so without this the cache would keep returning stale members.
