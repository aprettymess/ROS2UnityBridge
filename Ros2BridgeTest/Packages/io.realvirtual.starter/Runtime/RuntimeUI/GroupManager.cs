       ...
       // public bool OnlyVisualGroups=false;
        ...
        public List<GameObject> GetGOlist(string groupname)
        {
            ...
            if (allGroups.ContainsKey(groupname))
            {
                ...
                {
                    // Guard against destroyed Group components (e.g. short-lived MUs that carry a Group).
                    if (group != null)
        ...
        private Hashtable GetAllGroups() // index groupname , value list of gameobjects with the same groupname
        {
           ...
           /* if(OnlyVisualGroups)
            ...
            {
                // Skip destroyed Group components. The editor group cache can hold stale references
                // from a previous session, and short-lived MUs that carry a Group may already be gone.
