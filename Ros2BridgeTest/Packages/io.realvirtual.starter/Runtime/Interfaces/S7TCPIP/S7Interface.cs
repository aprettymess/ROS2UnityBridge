...
// This product bundles Sharp7, Copyright (C) 2016 Davide Nardella. It is individually
// licensed from Davide Nardella to in2sight GmbH under the Apache 2.0 license.
// For details on Sharp7 see http://snap7.sourceforge.net/
...
namespace realvirtual
{
    ...
    {
        [InfoBox("This product bundles Sharp7, Copyright (C) 2016 Davide Nardella. It is individually licensed from Davide Nardella to in2sight GmbH under the Apache 2.0 license. For details on Sharp7 see http://snap7.sourceforge.net/")]
        ...
        internal int GetArea(string name)
        {
        ...
        internal InterfaceSignal.DIRECTION GetDirection(string name)
        {
        ...
        internal static S7InterfaceSignal.S7TYPE GetDBType(string name)
        {
            // get pos of ".DB"
        ...
        internal static int GetDBMemPos(string name)
        {
            var pos = name.IndexOf(".DB");
        ...
        internal static S7InterfaceSignal.S7TYPE GetTypeFromName(string name)
        {
            S7InterfaceSignal.S7TYPE type = S7InterfaceSignal.S7TYPE.UNDEFINED;
            var toupper = name.ToUpper();
            var firsttwo = toupper.Substring(0, 2);
        ...
        internal static int GetFirstNum(string str, int start)
        {
            string res = "";
        ...
        internal int GetNumberAfterPoint(string str)
        {
            // find first point
        ...
        internal int GetNumberAfterLastPoint(string str)
        {
            // find first point
        ...
        internal static int GetLenght(string name)
        {
            int len;
            var toupper = name.ToUpper();
        ...
        private void ConnectS7Async()
        {
            ...
            try
            {
                ...
                {
                    int ExecTime = Client.ExecTime();
                    // Use ThreadStatus instead of Log() - Log() accesses gameObject which
                    // cannot be called from the background thread
            ...
            finally
            {
                ...
                _lastReconnectTime = DateTime.Now; // Reset timer after attempt completes
            }
        }
        ...
        internal int GetByteLengthForSignalType(S7InterfaceSignal.S7TYPE signalType)
        {
            switch (signalType)
            {
                case S7InterfaceSignal.S7TYPE.BOOL:
        ...
        private void FixedUpdate()
        {
            ...
            {
                OnDisconnected();
                // Don't call CloseInterface() - keep the communication thread alive
                // so it can handle reconnection via CommunicationThreadUpdate()
                _isdisconnected = true;
