...
{
    //! Base class for all realvirtual communication interfaces providing common functionality for PLC and system connections.
    //! Manages signal collection, initialization, and lifecycle for derived interface implementations.
    //! Automatically discovers and manages all child signal objects (PLCInput/PLCOutput) for data exchange.
    //! This abstract base class provides the foundation for creating custom interfaces to connect with external systems.
    //! Derive from this class to implement new communication protocols or integrate with proprietary automation systems.
    [HelpURL("https://doc.realvirtual.io/components-and-scripts/interfaces/custom-interfaces")]
    public class InterfaceBaseClass : realvirtualBehavior, IPostAllScenesLoaded
    {
        //! Display name for interface in UI lists (removes "Interface" suffix by default, can be overridden for custom names)
        public virtual string InterfaceDisplayName => GetType().Name.Replace("Interface", "");

        [ReadOnly] [ShowIf("ShowIsConnectedField")] public bool IsConnected=false;

        protected virtual bool ShowIsConnectedField => true;

        public List<InterfaceSignal> InterfaceSignals = new List<InterfaceSignal>();
        public Hashtable SignalNamesOutputs = new Hashtable();
        public Hashtable SignalNamesInputs = new Hashtable();
        private bool standardinitpassed = false;
        ...
        public void UpdateInterfaceSignals(ref int inputs, ref int outputs)
        {
            ...
            SignalNamesInputs.Clear();
            SignalNamesOutputs.Clear();
            inputs = 0;
            outputs = 0;
            ...
            foreach (Signal signal in signals)
            {
                ...
                {
                    inputs++;
                    SignalNamesInputs.Add(signal.GetSignalName(), newsignal);
                }
                if (newsignal.Direction == InterfaceSignal.DIRECTION.OUTPUT)
                {
                    outputs++;
                    SignalNamesOutputs.Add(signal.GetSignalName(), newsignal);
                }
            }
        ...
        public Signal CreateSignalObject(string name, SIGNALTYPE type, SIGNALDIRECTION direction)
        {
            ...
            if (signal == null)
            {
                signal = new GameObject(name);  // Fixed: use the variable name, not literal "name"
            ...
            if (direction == SIGNALDIRECTION.INPUT)
            {
                ...
                switch (type)
                {
                    case SIGNALTYPE.BOOL:
                        ...
                        {
                            newsignal = signal.AddComponent<PLCInputBool>();
                        }
                    ...
                    case SIGNALTYPE.INT:
                        ...
                        {
                            newsignal = signal.AddComponent<PLCInputInt>();
                        }
                    ...
                    case SIGNALTYPE.REAL:
                        if (signal.GetComponent<PLCInputFloat>() == null)
                        {
                            newsignal = signal.AddComponent<PLCInputFloat>();
                        }
                        break;
                    case SIGNALTYPE.TEXT:
                        if (signal.GetComponent<PLCInputText>() == null)
                        {
                            newsignal = signal.AddComponent<PLCInputText>();
                        }
                        break;
                } 
            }
            ...
            if (direction == SIGNALDIRECTION.OUTPUT)
            {
                switch (type)
                {
                    case SIGNALTYPE.BOOL:
                        ...
                        {
                            newsignal = signal.AddComponent<PLCOutputBool>();
                        }
                    ...
                    case SIGNALTYPE.INT:
                        ...
                        {
                            newsignal = signal.AddComponent<PLCOutputInt>();
                        }
                    ...
                    case SIGNALTYPE.REAL:
                        ...
                        {
                            newsignal = signal.AddComponent<PLCOutputFloat>();
                        }

                        break;
                    case SIGNALTYPE.TEXT:
                        if (signal.GetComponent<PLCOutputText>() == null)
                        {
                            newsignal = signal.AddComponent<PLCOutputText>();
                        }
        ...
        protected  void OnConnected()
        {
            ...
            if (realvirtualController!=null)
                  realvirtualController.OnConnectionOpened(gameObject);
        }
        ...
        protected  void OnDisconnected()
        {
            ...
            IsConnected = false;
            if (realvirtualController!=null)
                realvirtualController.OnConnectionClosed(gameObject);
        }
        ...
        public virtual void PostAllScenesLoaded()
        {
            standardinitpassed = true;
