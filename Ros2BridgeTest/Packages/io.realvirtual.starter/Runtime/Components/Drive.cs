...
#pragma warning disable 0219
    ...
    [SelectionBase]
    [ExecuteInEditMode]
    [Icon("Packages/io.realvirtual.starter/Editor/EditorAssets/EditorScriptIcons/Drive Icon.png")]
    #region doc
    ...
    public class Drive : BaseDrive, IXRPlaceable, IEditModeFinished, ITimeSyncedPhysics
    {
        ...
        [HideInInspector] public bool IsExternallyControlled = false; //!< If true, skip FixedUpdate calculations (position set externally by IK or other systems)
        ...
        private bool _speedStretched = false; //!< True while TargetSpeed/Acceleration are temporarily reduced by a timed (synchronized PTP) move, so the nominal values are only captured once and reliably restored
        // XDE Integration
#if REALVIRTUAL_INTERACT
        [HideInInspector]
        ...
        // IK optimization: track last applied position to skip unnecessary transform updates
        private float _lastAppliedTransformPosition = float.NaN;
        #endregion
        ...
        {
            // If a previous timed (synchronized PTP) move left the speed/acceleration stretched,
            // restore the nominal values first so this (untimed, e.g. blending RedirectDriveTo) move
            // runs at — and re-saves — the true nominal, not an already-reduced value (prevents runaway).
            if (_speedStretched)
            {
                TargetSpeed = StandardSpeed;
                Acceleration = StandardAcceleration;
                _speedStretched = false;
            }
        ...
        //! Redirects the drive to a new target position while preserving current speed.
        //! Used during blending to avoid speed discontinuity. Clears both acceleration flags
        //! so the stoppos-based direction logic in CalcFixedUpdate correctly determines whether
        //! to accelerate or brake based on destination vs. stopping position.
        //! For SmoothAcceleration drives, these flags are unused - InitSmoothDriveTo handles
        //! velocity preservation automatically via SetInitialVelocity(CurrentSpeed).
        public void RedirectDriveTo(float Target)
        {
        ...
        public void DriveTo(float Target, float time)
        {
            ...
            BlockDestination = false;
            // Capture the nominal speed/acceleration only ONCE, before any stretch overwrites them.
            // Re-capturing on every timed/blended move would save an already-reduced value and the
            // restore would never return to the true nominal — causing a progressive slowdown (runaway).
            if (!_speedStretched)
            {
                StandardSpeed = TargetSpeed;
            ...
            else if(!SmoothAcceleration)
            {
                ...
                {
                    // Calc Acceleration — stretches the move to last exactly 'time' (synchronized PTP).
                    if (UseAcceleration)
                    {
        ...
        //! Sets the position of the drive and updates the visual position.
        //! When precision > 0, skips update if position change is smaller than precision (optimization for high-frequency updates like IK).
        //! Returns true if position was updated, false if skipped due to precision threshold.
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
        public bool SetPosition(float position, float precision = 0)
        {
            // When precision > 0, skip if position hasn't changed significantly (IK optimization)
            if (precision > 0)
            {
                float delta = position - _lastAppliedTransformPosition;
                if (delta > -precision && delta < precision)
                    return false;

                _lastAppliedTransformPosition = position;
                CurrentPosition = position;
                IsPosition = position;

                // Fast path for standard transform-based rotation (most common for robot axes)
                if (!_istransportsurface && IsRotation && !MoveThisRigidBody && Direction != DIRECTION.Virtual)
                {
                    transform.localRotation = _localstartquat * Quaternion.Euler(_localdirection * (position + Offset));
                    return true;
                }

                // Fast path for standard transform-based linear movement
                if (!_istransportsurface && !IsRotation && !MoveThisRigidBody && Direction != DIRECTION.Virtual)
                {
                    transform.localPosition = _localstartpos +
                                             _positiondirection *
                                             ((position + Offset) / _controllerscale) /
                                             _localscale * initialScale;
                    return true;
                }

                // Fallback to full SetPosition() for complex cases
                SetPosition();
                return true;
            }

            // Standard path (no precision threshold)
        ...
        public void CalcFixedUpdate()  // changed to public in case of need for external controlling it
        {
            ...
            // Skip position calculation for externally controlled drives (IK, etc.)
            // Position is set directly via SetPositionIK() - no internal movement logic needed
        ...
        //! Self-manages multiuser behavior. When not owner, enables PositionOverwrite and ForceStop
        //! to accept externally synced position/speed without internal motion calculations.
