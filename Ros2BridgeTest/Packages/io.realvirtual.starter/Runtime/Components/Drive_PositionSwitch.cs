...
namespace realvirtual
{
    ...
    public class Drive_PositionSwitch : BehaviorInterface, IDriveBehavior
    {
        ...
        private bool _rangeWarned; //!< Ensures the invalid-limits warning is only logged once
        ...
        public void CalcFixedUpdate()
        {
            ...
            if (_drive.UseLimits && _drive.JumpToLowerLimitOnUpperLimit)
            {
                ...
                if (range <= 0)
                {
                    // Guard against misconfigured limits - the while loops below would never terminate
                    if (!_rangeWarned)
                    {
                        _rangeWarned = true;
                        Logger.Warning("Drive_PositionSwitch - UpperLimit must be greater than LowerLimit on the Drive, position normalization is skipped", this);
                    }
                }
                else
                {
