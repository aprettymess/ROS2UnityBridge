...
namespace realvirtual
{
    ...
    [Icon("Packages/io.realvirtual.starter/Editor/EditorAssets/EditorScriptIcons/Transportsurface Icon.png")]
    #region doc
    ...
    public class TransportSurface : BaseTransportSurface, IGuidedSurface, IXRPlaceable, ITimeSyncedPhysics
    {
        ...
        private bool _parentTrackingInitialized = false;
        private Transform _parentDriveTransform;
        private Vector3 _cachedLocalAxis;
        private bool _radialParentErrorLogged = false;
        ...
        private void HandleEditorColliderSwitching()
        {
            ...
            Global.SetLayerSafe(gameObject, Layer);
        }
        ...
        private void AfterDriveStartInit(Drive drive)
        {
            ...
            // Cache local axis for radial physics (avoid computing every FixedUpdate)
            if (Radial)
            {
                _cachedLocalAxis = TransportDirection.normalized;
            }
        ...
        void Start()
        {
            ...
            // Cache parent transform reference to avoid repeated property access in FixedUpdate
            if (parentDriveNotNull)
            {
                _parentDriveTransform = ParentDrive.transform;
            }
 ...
 -global * speed * realvirtualController.SpeedOverride / realvirtualController.Scale;
        ...
        {
            if (_parentTrackingInitialized) return;

            parentposbefore = _parentDriveTransform.position;
            parentrotbefore = _parentDriveTransform.localRotation;
            _parentTrackingInitialized = true;
        }

        private bool HasParentMoved()
        {
            return parentposbefore != _parentDriveTransform.position ||
                   parentrotbefore != _parentDriveTransform.localRotation;
        }

        private (Vector3 deltaUp, Vector3 deltaArea) CalculateParentDelta()
        {
            var parentpos = _parentDriveTransform.position;
        ...
        private void UpdateRadialPhysics(float deltaTime)
        {
            if (parentDriveNotNull)
            {
                // Log error only once to avoid performance impact
                if (!_radialParentErrorLogged)
                {
                    Logger.Error("Radial conveyor with parent drive not implemented!", this, false);
                    _radialParentErrorLogged = true;
                }
                return;
            }
            ...
            // Use cached local axis instead of computing TransformDirection then InverseTransformVector every frame
            var angle = speed * deltaTime * realvirtualController.SpeedOverride * SpeedScaleTransportSurface;

            _rigidbody.rotation = _rigidbody.rotation * Quaternion.AngleAxis(-angle, _cachedLocalAxis);
            _rigidbody.MoveRotation(_rigidbody.rotation * Quaternion.AngleAxis(angle, _cachedLocalAxis));
        }

        private Vector3 GetVertikalMov(Vector3 deltacomplete)
        {
            // Isolate movement along primary axes to prevent rounding errors
            // Use direct component checks instead of expensive Vector3.Angle() calls
            if (deltacomplete.x == 0 && deltacomplete.z == 0)
                return new Vector3(0, deltacomplete.y, 0);

            if (deltacomplete.y == 0 && deltacomplete.z == 0)
