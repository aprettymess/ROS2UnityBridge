...
namespace realvirtual
{
    ...
    //! 
    //! For detailed documentation see: https://doc.realvirtual.io/components-and-scripts/mu-movable-unit/source
    #endregion
    [HelpURL("https://doc.realvirtual.io/components-and-scripts/mu-movable-unit/source")]
    [Icon("Packages/io.realvirtual.starter/Editor/EditorAssets/EditorScriptIcons/Source Icon.png")]
    public class Source : BaseSource,ISignalInterface,IXRPlaceable
    {
        ...
        [Tooltip("Enable object pooling for better performance (reuses MUs instead of instantiating)")]
        public bool UsePooling = false; //!< Enable object pooling for MU generation
        [ShowIf("UsePooling")]
        [Tooltip("Initial pool size - objects are pre-instantiated at start")]
        public int PoolSize = 50; //!< Initial size of the object pool
        [ShowIf("UsePooling")]
        [Tooltip("Pre-instantiate pool objects during Start (reduces runtime allocation)")]
        public bool PrewarmPool = true; //!< Pre-instantiate pool objects at start
        [ShowIf("UsePooling")]
        [Tooltip("Allow pool to grow beyond initial size if needed")]
        public bool AllowPoolGrowth = true; //!< Allow pool to grow if all objects are in use
        [ShowIf("UsePooling")][ReadOnly]
        public int PooledCount = 0; //!< Current number of objects in the pool
        [ShowIf("UsePooling")][ReadOnly]
        public int ActiveCount = 0; //!< Current number of active pooled objects
        ...
        private Dictionary<int, bool> originalRendererStates = new Dictionary<int, bool>(); // Store by index since renderers change
        private Dictionary<int, bool> originalGameObjectActiveStates = new Dictionary<int, bool>(); // Store child active states

        // Object pooling
        private Stack<MU> _pool = new Stack<MU>();
        private Transform _poolContainer;

        //! Static flag set by DESManager when running in FastForward mode.
        //! Allows Source.cs (in the starter package) to skip visual ops without a direct dependency on DESManager.
        //! DESManager sets this to true in DisableRenderingAndAudio() and false in EnableRenderingAndAudio().
        public static bool IsDESFastForward = false;
        ...
        public void OnMUDelete(MU mu)
        {
            ...
            // Clean up from generated list to prevent memory leak
            _generated.Remove(mu.gameObject);
        }
        ...
        //! Initializes the object pool with pre-instantiated MUs
        private void InitializePool()
        {
            if (!UsePooling) return;

            // Create a hidden container for pooled objects at scene root (prevents orphaning if Source is destroyed)
            var containerObj = new GameObject($"{name}_Pool");
        ...
        //! Creates a new MU for the pool (internal instantiation)
        private MU CreatePooledMU()
        {
            if (ThisObjectAsMU == null)
            {
                Logger.Warning("Cannot create pooled MU: Template object is null", this);
                return null;
            }

            GameObject newmu = Instantiate(ThisObjectAsMU, Vector3.zero, Quaternion.identity);
            newmu.SetActive(false);
            newmu.transform.SetParent(_poolContainer);

            // Setup MU component
            MU mu = newmu.GetComponent<MU>();
            if (mu == null)
            {
                mu = newmu.AddComponent<MU>();
            }
            ...
            if (!UseAGXPhysics)
            {
                Rigidbody rb = newmu.GetComponentInChildren<Rigidbody>();
                if (rb == null)
                    rb = newmu.AddComponent<Rigidbody>();
                rb.mass = Mass;
                if (SetCenterOfMass)
            ...
            // Setup layer
            if (GenerateOnLayer != "" && LayerMask.NameToLayer(GenerateOnLayer) != -1)
            {
            ...
            if (ChangeDefaultLayer)
            {
                ...
                if (box != null && box.gameObject.layer == LayerMask.NameToLayer("Default"))
                    box.gameObject.layer = LayerMask.NameToLayer("rvMU");

                var mesh = newmu.GetComponentInChildren<MeshCollider>();
                if (mesh != null && mesh.gameObject.layer == LayerMask.NameToLayer("Default"))
                    mesh.gameObject.layer = LayerMask.NameToLayer("rvMUTransport");
            }

            // Remove Source component from pooled object
        ...
        //! Gets an MU from the pool or creates a new one if pool is empty
        private MU GetFromPool()
        {
            MU mu;
            ...
            else
            {
                ...
                Logger.Warning("Pool exhausted and growth disabled. Cannot generate MU.", this);
                return null;
            }
        ...
        //! Returns an MU to the pool for reuse
        public void ReturnToPool(MU mu)
        {
            if (mu == null) return;

            // Remove from generated list
            if (_generated.Contains(mu.gameObject))
                _generated.Remove(mu.gameObject);

            // Reset MU state
            mu.ResetForPool();

            // Skip visual ops in FastForward — reconciled by DESManager.ReconcileAllMUs()
            if (!IsDESFastForward)
            {
                mu.gameObject.SetActive(false);
                mu.transform.SetParent(_poolContainer, worldPositionStays: false);
                mu.transform.localPosition = Vector3.zero;
                mu.transform.localRotation = Quaternion.identity;
            }

            // Return to pool (logical state — always updated)
            _pool.Push(mu);
            PooledCount = _pool.Count;
            ActiveCount--;
        }

        //! Generates an MU from the pool (fast path avoiding Instantiate)
        private MU GenerateFromPool()
        {
            MU mu = GetFromPool();
            if (mu == null)
                return null;

            GameObject newmu = mu.gameObject;

            // Skip all visual/physics ops in FastForward — no rendering, reconciled on mode switch
            if (!IsDESFastForward)
            {
                // Position and activate
                newmu.transform.SetParent(Destination != null ? Destination.transform : null);
                newmu.transform.position = transform.position;
                newmu.transform.rotation = transform.rotation;
                newmu.transform.localScale = transform.localScale;

                // Reset rigidbody velocity and constraints
                if (!UseAGXPhysics)
                {
                    Rigidbody rb = newmu.GetComponent<Rigidbody>();
                    if (rb != null)
                    {
                        ...
                        // Reset constraints - source template has FreezeAll due to FreezeSourcePosition
                        rb.constraints = rbconstraints;
                    }
                }

                // Enable colliders
                Collider[] colliders = newmu.GetComponentsInChildren<Collider>(true);
                ...
                newmu.SetActive(true);

                // Restore original states (respects intentionally disabled children/renderers)
                RestoreTemplateGameObjectStates(newmu); // Restore child active states first
                RestoreTemplateRendererStates(newmu);   // Then restore renderer enabled states
            }

            // Initialize MU with new ID
            Created++;
            ID++;
            mu.InitMu(name, ID, realvirtualController.GetMUID(newmu));
            mu.CreatedBy = this;

            // Skip component lookups and event invocations in FastForward
            if (!IsDESFastForward)
            {
                // Check for child MUs
        ...
        //! Ensures all pooled (inactive) MUs are SetActive(false) and parented to the pool container.
        //! Called by DESManager.ReconcileAllMUs() after leaving FastForward mode to restore pool state.
        public void ReconcilePoolState()
        {
            if (!UsePooling || _poolContainer == null) return;

            // Iterate the pool stack — all entries should be inactive and under _poolContainer
            foreach (var mu in _pool)
            {
                if (mu == null) continue;
                if (mu.gameObject.activeSelf)
                    mu.gameObject.SetActive(false);
                if (mu.transform.parent != _poolContainer)
                    mu.transform.SetParent(_poolContainer, worldPositionStays: false);
            }
        }
        ...
        {
            // Clean up pool when source is destroyed
            if (UsePooling)
                ClearPool();
        }
        ...
        //! Stores the initial enabled state of all renderers in the template (before DontVisualize is applied)
        private void StoreTemplateRendererStates()
        {
            if (ThisObjectAsMU == null) return;

            originalRendererStates.Clear();
            Renderer[] renderers = ThisObjectAsMU.GetComponentsInChildren<Renderer>(true);
        ...
        //! Restores the original enabled state of renderers on a generated MU
        private void RestoreTemplateRendererStates(GameObject generatedMU)
        {
            if (originalRendererStates.Count == 0)
            {
                // Fallback: enable all if no states stored
                Renderer[] allRenderers = generatedMU.GetComponentsInChildren<Renderer>(true);
            ...
            Renderer[] renderers = generatedMU.GetComponentsInChildren<Renderer>(true);

            for (int i = 0; i < System.Math.Min(originalRendererStates.Count, renderers.Length); i++)
            {
                if (originalRendererStates.TryGetValue(i, out bool wasEnabled))
                {
        ...
        //! Stores the initial active state of all child GameObjects in the template
        private void StoreTemplateGameObjectStates()
        {
            if (ThisObjectAsMU == null) return;

            originalGameObjectActiveStates.Clear();
            Transform[] transforms = ThisObjectAsMU.GetComponentsInChildren<Transform>(true);

            for (int i = 0; i < transforms.Length; i++)
            {
                if (transforms[i] != null)
                {
                    // Store the local active state (activeSelf), not the hierarchy state (activeInHierarchy)
                    originalGameObjectActiveStates[i] = transforms[i].gameObject.activeSelf;
                }
            }
        }

        //! Restores the original active state of child GameObjects on a generated MU
        private void RestoreTemplateGameObjectStates(GameObject generatedMU)
        {
            if (originalGameObjectActiveStates.Count == 0) return;

            Transform[] transforms = generatedMU.GetComponentsInChildren<Transform>(true);

            for (int i = 0; i < System.Math.Min(originalGameObjectActiveStates.Count, transforms.Length); i++)
            {
                if (originalGameObjectActiveStates.TryGetValue(i, out bool wasActive))
                {
                    transforms[i].gameObject.SetActive(wasActive);
                }
            }
        }
        ...
        protected void Start()
        {
            ...
            // Store template states BEFORE changing visibility/colliders
            StoreTemplateGameObjectStates(); // Store child active states first
            StoreTemplateRendererStates(); // Store renderer states before DontVisualize disables them
            StoreTemplateColliderStates(); // Store collider states before disabling
            ...
            // Initialize object pool if pooling is enabled
            InitializePool();

            // Start interval generation AFTER pool is ready (fixes race condition)
            if (Interval > 0)
            {
          ...
          UseAGXPhysics = false;
            ...
            if (Enabled)
            {
                ...
                if (source != null)
                {
                    ...
                    source.RestoreTemplateColliderStates(newmu);
        ...
        //! Self-manages multiuser behavior. When not owner, disables MU generation
        //! so that MUs are only created by the server and synced to clients.
