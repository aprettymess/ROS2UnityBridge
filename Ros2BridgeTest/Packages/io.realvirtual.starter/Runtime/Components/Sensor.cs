...
namespace realvirtual
{
        ...
        Sensor : BaseSensor, ISignalInterface, IXRPlaceable
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            // A sensor must never detect another sensor: both live on the rvSensor layer, which
            // collides with itself in the collision matrix, so two overlapping sensor trigger boxes
            // would otherwise report each other as "occupied". Ignore colliders belonging to a Sensor.
            if (other.GetComponent<Sensor>() != null)
                return;
            var tmpcolliding = CollidingObjects;
