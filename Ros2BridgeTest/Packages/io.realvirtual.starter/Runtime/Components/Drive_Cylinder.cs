...
namespace realvirtual
{
    ...
    [Icon("Packages/io.realvirtual.starter/Editor/EditorAssets/EditorScriptIcons/Drive_Cylinder Icon.png")]
    public class Drive_Cylinder : BehaviorInterface, IDriveBehavior
    {
        ...
        public void CalcFixedUpdate()
        {
            ...
            // Set isOut/isIn based on actual position and clear stuck moving flags
            if (Cylinder.IsPosition == MaxPos)
            {
                _isOut = true;
                _isIn = false;
                _movingOut = false;
            }
            if (Cylinder.IsPosition == MinPos)
            {
                _isIn = true;
                _isOut = false;
                _movingIn = false;
