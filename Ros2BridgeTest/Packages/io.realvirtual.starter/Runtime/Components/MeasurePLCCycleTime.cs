...
namespace realvirtual
{
    public class MeasurePLCCycleTime : MonoBehaviour
    {
        ...
        [Tooltip("Warmup time in seconds after first PLC counter change before starting min/max/med measurements")]
        ...
        private bool firstChangeDetected;
        ...
        void FixedUpdate()
        {
            ...
            if (SignalPLCCycleCounter != null)
            {
                ...
                {
                    // Start warmup timer on first PLC counter change
                    if (!firstChangeDetected)
                    {
                        firstChangeDetected = true;
                        warmupEndTime = Time.fixedTime + WarmupTimeSeconds;
                    ...
                    if (!warmupComplete)
                    {
                        if (Time.fixedTime >= warmupEndTime)
                        {
                            warmupComplete = true;
                            // Reset all stats after warmup
                            MinCommCycleTimeMs = 999999;
                            MaxCommCycleTimeMs = 0;
                            sumcycletime = 0;
                            nummeasures = 0;
