...
#pragma warning disable CS3001, CS3002, CS3003, CS3009
    ...
    public class Axis : MonoBehaviour
    {
...
#if UNITY_EDITOR
        private List<GameObject> cachedGroupMeshes;
        private string cachedGroupName;
        private int cachedGroupsVersion = -1;
#endif
        void OnDrawGizmos()
        {
#if UNITY_EDITOR 
        ...
        public void DisplayGroupMeshes(Color color, bool forcecolor = false )
        {
            ...
            var objs = GetAllMeshesWithGroupCached(groupname);
            ...
            bool checkHover = Global.g4acontrollernotnull;
            foreach (var obj in objs)
            {
                var meshFilter = obj.GetComponent<MeshFilter>();
                if (meshFilter == null) continue;
                if (checkHover && Global.realvirtualcontroller.CheckIfMeshIsHovered(obj))
                    continue;
        ...
        private void Awake()
        {
            ...
            else
            {
                ...
                if (drive.UseLimits != UseLimits ||
                    (UseLimits && (drive.UpperLimit != UpperLimit || drive.LowerLimit != LowerLimit)))
                {
                    Logger.Warning($"Axis [{gameObject.name}] overwrites Drive limit settings (UseLimits: {drive.UseLimits}->{UseLimits}, Lower: {drive.LowerLimit}->{LowerLimit}, Upper: {drive.UpperLimit}->{UpperLimit}). Axis is the master for limits when present.", this);
                }
        ...
        public void OnDestroy()
        {
            ...
            if (Kinematic != null)
            {
                var groupName = Kinematic.GetGroupName();
                if (string.IsNullOrEmpty(groupName)) return;
                var comps = Kinematic.GetAllWithGroup(groupName);
...
#if UNITY_EDITOR
        private List<GameObject> GetAllMeshesWithGroupCached(string group)
        {
            int currentVersion = Groups.GetCachedGroupsVersion();
            if (cachedGroupMeshes != null && cachedGroupName == group && cachedGroupsVersion == currentVersion)
                return cachedGroupMeshes;

            cachedGroupName = group;
            cachedGroupsVersion = currentVersion;
            if (cachedGroupMeshes == null)
                cachedGroupMeshes = new List<GameObject>();
            else
                cachedGroupMeshes.Clear();
            ...
            foreach (var groupcomp in groupcomps)
            {
                if (groupcomp.GetGroupName() == group)
                {
                    ...
                    {
                        cachedGroupMeshes.Add(groupcomp.gameObject);
                    }
                }
            }
