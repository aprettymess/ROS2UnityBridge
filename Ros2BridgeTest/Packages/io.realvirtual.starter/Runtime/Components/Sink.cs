...
namespace realvirtual
{
    ...
    [HelpURL("https://doc.realvirtual.io/components-and-scripts/mu-movable-unit/sink")]
    [SelectionBase]
    #if !REALVIRTUAL_AGX
    ...
    //! 
    //! For detailed documentation see: https://doc.realvirtual.io/components-and-scripts/mu-movable-unit/sink
    #endregion
    public class Sink : realvirtualBehavior
    {
            ...
            foreach (var obj in tmpcolliding.ToArray())
            {
                ...
                if (mu != null)
                {
                    if (DeleteOnlyTag == "" || (mu.gameObject.tag == DeleteOnlyTag))
                    {
                        ...
                        if (!Dissolve)
                            mu.ReturnToPool(); // Returns to pool if pooled, otherwise destroys
                        else
                            mu.Dissolve(DestroyFadeTime); // Dissolve uses ReturnToPool internally
                        SumDestroyed++;
                    }
                }
        ...
        //! Self-manages multiuser behavior. When not owner, disables MU deletion
        //! so that MU lifecycle is controlled by the server.
