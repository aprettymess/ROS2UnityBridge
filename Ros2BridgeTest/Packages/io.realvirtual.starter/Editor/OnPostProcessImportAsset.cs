...
#pragma warning restore CS0618
        ...
        static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
        {
            ...
            {
                // Only trigger on specific important files - check for package paths
                if (str.Contains("Global.cs") && str.Contains("io.realvirtual"))
                {
                ...
                else if (str.Contains("realvirtual.editor.asmdef") && str.Contains("io.realvirtual"))
                {
                    Game4AutomationImport = true;
                }
            }
            ...
            if (Game4AutomationImport && !Application.isPlaying)
            {
                ...
                // Disable Interact
                string MenuName = "Tools/realvirtual/Enable Interact (Pro)";
                ...
                // Check and set Professional version define - check if Professional package is installed
                if (AssetDatabase.IsValidFolder(RealvirtualAssetPaths.ProfessionalPackage))
                {
                ...
                EditorApplication.delayCall += () =>
                {
                    RealvirtualGetStartedWindow.ShowWindow();
                };

                // Legacy cleanup no longer needed with UPM packages
